		...
		<div class="card-body">
			<div class="tab-container">
				<ul class="nav nav-tabs" id="tab">
					@foreach($role as $i => $s)
					<li class="t t-{{str_replace(' ', '_', $s->name)}}"><a id="t-{{str_replace(' ', '_', $s->name)}}" href="#{{str_replace(' ', '_', $s->name)}}" data-toggle="tab">{{strtoupper($s->name)}}</a></li>
					@endforeach
				</ul>
				<div class="tab-content">
					...
					<div id="{{str_replace(' ', '_', $s->name)}}" class="tab-pane @if($i==0) active @endif cont">
						<table class="table table-curved text-center dataTable-{{$s->id}}" id="dataTable" width="100%" cellspacing="0">
							<thead>
